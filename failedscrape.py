...
states_fullname = [
'alabama', 'alaska', 'arkansas', 'arizona', 'california', 'colorado', 'connecticut', 'delware', 'florida', 'georgia', 'hawaii', 'idaho', 'illinois',
...
for states in states_fullname:
  ...
  containers = page_soup.findAll("table", {"id": "usa_table_countries_yesterday"})
  container = containers[0].tbody.tr
  temp_list = container.findAll("td")
  num0 = str(temp_list[1])
  num1 = num0.replace('<td>', '')
  num = num1.replace('</td>', '')

  sheet[str(code) + str(c)].value = num
  c += 1
